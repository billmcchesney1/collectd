/**
 * collectd - src/utils_format_syslog.c
...
#define SYSLOG_FORBIDDEN " \t\"\\:!/()\n\r"

/* Utils functions to format data sets in syslog format.
 * Largely taken from utils_format_syslog.c as it contains similar constructs */

 static int syslog_format_values (char *ret, size_t ret_len,
...
#define BUFFER_ADD(...) do { \
 	...
 	if (status < 1) \
 		...
 		{
 			ERROR ("syslog_format_values plugin: Unknown data source type: %i",
 				ds->ds[ds_num].type);
 			return (-1);
 		}
...
#undef BUFFER_ADD
 	...
 	static void syslog_copy_escape_part_jmx (char *dst, const char *src, size_t dst_len,
 		char escape_char)
 	{
 		size_t i, j;
 	...
 	static void syslog_copy_escape_part (char *dst, const char *src, size_t dst_len,
 		char escape_char)
 	{
 		size_t i;
 	...
 	static int syslog_format_tags (char *ret, int ret_len,
    ...
    sstrncpy (plugin_instance_to_process, vl->plugin_instance, sizeof (plugin_instance_to_process));
 		...
 		if (plugin_instance_from_plugin_instance == NULL){
 		    syslog_copy_escape_part (n_plugin_instance, vl->plugin_instance,
 			    sizeof (n_plugin_instance), escape_char);
 		}
 		else{
 		    syslog_copy_escape_part (n_plugin_instance, plugin_instance_from_plugin_instance,
 			    sizeof (n_plugin_instance), escape_char);
 		}

 		syslog_copy_escape_part (n_plugin, vl->plugin,
 	...
 	static int syslog_format_name_jmx (char *ret, int ret_len,
 		value_list_t const *vl,
 		char const *prefix,
 		char const escape_char,
 		unsigned int flags)
	...
	//char n_host[DATA_MAX_NAME_LEN];
 		...
 		syslog_copy_escape_part_jmx (n_plugin, vl->plugin,
 			sizeof (n_plugin), escape_char);
 		syslog_copy_escape_part_jmx (n_type, vl->type,
 			sizeof (n_type), escape_char);
 		syslog_copy_escape_part_jmx (n_type_instance, vl->type_instance,
 			sizeof (n_type_instance), escape_char);
 		syslog_copy_escape_part_jmx (n_plugin_instance, vl->plugin_instance,
 			sizeof (n_plugin_instance), escape_char);
 	...
 	static int syslog_format_name (char *ret, int ret_len,
 		value_list_t const *vl,
 		char const *ds_name,
 		char const *prefix,
 		char const escape_char,
	...
	//char n_host[DATA_MAX_NAME_LEN];
 		if (0 == strcmp("GenericJMX", vl->plugin)){
 			return syslog_format_name_jmx(ret, ret_len, vl, prefix, escape_char, flags);
 		}
 		...
 		syslog_copy_escape_part (n_plugin, vl->plugin,
 				sizeof (n_plugin), escape_char);			
 		syslog_copy_escape_part (n_type, vl->type,
 			sizeof (n_type), escape_char);
 		syslog_copy_escape_part (n_type_instance, vl->type_instance,
 			sizeof (n_type_instance), escape_char);
 		sstrncpy (tmp_type, n_type, sizeof (tmp_type));
 	...
 	static void escape_syslog_string (char *buffer, char escape_char)
 	{
 		char *head;
 	...
 	int format_syslog (char *buffer, size_t buffer_size,
 		...
 		unsigned int flags)
 	{
		...
		/* Copy the identifier to `key' and escape it. */
 			status = syslog_format_name (key, sizeof (key), vl, ds_name,
 				prefix, escape_char, flags);
 			if (status != 0)
 			{
 				ERROR ("format_syslog: error with syslog_format_name");
 				sfree (rates);
 				return (status);
 			}

 			escape_syslog_string (key, escape_char);
		/* Convert the values to an ASCII representation and put that into
		 * `values'. */
 			status = syslog_format_values (values, sizeof (values), i, ds, vl, rates);
 			if (status != 0)
 			{
 				ERROR ("format_syslog: error with syslog_format_values");
 				sfree (rates);
 				return (status);
 			}

		/* Copy the identifier to `tags' and escape it. */
 			status = syslog_format_tags (final_tags, sizeof (final_tags), vl,
 				tags, escape_char, flags);
 			if (status != 0)
 			{
 				ERROR ("format_syslog: error with syslog_format_tags");
 				sfree (rates);
 				return (status);
 			}

		/* Compute the syslog command */
 			...
 			DEBUG(
 				"final tag format is : cosmos: %u %s %s %s\r\n",
 				(unsigned int) CDTIME_T_TO_TIME_T (vl->time), 
 				key,
 				values,
 				final_tags);
 			if (message_len >= sizeof (message)) {
 				ERROR ("format_syslog: message buffer too small: "
		...
		/* Append it in case we got multiple data set */
 			...
 			{
 				ERROR ("format_syslog: target buffer too small");
 				sfree (rates);
 				return (-ENOMEM);
 			}
